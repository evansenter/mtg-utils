...
#
# None of these four are this branch's bytes. Each was generated by a checkout
# of the commit it is pinned against and then diffed, so a snapshot that moves
# under a refactor here is a real movement rather than a self-fulfilling one.
# They have been re-taken twice as main moved beneath the branch, because a
# deliberate change to a reported number on main has to arrive in the snapshot
# as main's answer:
#
#   colourless  4db2aa5, the commit before the optimisation pass
#   mono, partner   16f54c6, which modelled the ritual burst
#   multi       7e0a97b, which additionally read restricted mana per line and
#               so moved the deck's accelerant count from 13 to 14
